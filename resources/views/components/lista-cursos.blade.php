<div class="grid grid-cols-1  mx-0 sm:grid-cols-{{$sm}} md:grid-cols-{{$md}} lg:grid-cols-{{$lg}} xl:grid-cols-{{$xl}} ml:grid-cols-{{$ml}} text-center mt-3 justify-items-center">
    @foreach ($cursos as $curso)
        <div class="relative px-4  h-auto w-56 bg-white-700 dark:bg-bluedark  rounded-xl pt-7 sm:pb-4 my-0 mb-2">
            ...
            <div class="content-center text-center space-y-1 space-x-0 px-0 sm:text-center">
                ...
                <div class="dark:text-whitetext  flex justify-between mb-2">
                    ...
                    @auth
                        @livewire('like-curso', ['idAsig' => $curso->cursoAsignacion->idasig], key($curso->idcur))
                    @else
                        <div class="flex items-center">
