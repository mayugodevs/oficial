...
<div class="grid grid-cols-1 couse-center sm:grid-cols-{{$sm}} md:grid-cols-{{$md}} lg:grid-cols-{{$lg}} xl:grid-cols-{{$xl}} ml:grid-cols-{{$ml}} text-center">
    @foreach ($cursos as $curso)
        @if (!in_array($curso->idcur, $exclusiones))
                ...
                <div class="content-center text-center space-y-1 space-x-0 px-0 sm:text-center">
                    ...
                    <hr class="">
                        ...
                        @else
                            ...
                            <div class="flex justify-between mt-0 w-100">
                                @if (in_array($curso->idcur, $cursos_select))
                                    <div>
                                        <button class="bg-transparent p-1 py-1 px-1 hover:bg-red-600 text-gray-600 hover:text-whitetext   border-red-600 
                                            dark:text-whitetext duration-200 border-2  hover:border-transparent rounded" 
                                            wire:click="addCourse({{$curso->idcur}}, 1)">
                                            <p class="inline-flex font-semibold ">Quitar</p>
                                        </button>
                                    </div>                            
                                @else
                                    <div>
                                        <button class="bg-transparent p-1 py-1 px-1 hover:bg-green text-gray-600 hover:text-whitetext   
                                            border-green dark:text-whitetext duration-200 border-2  hover:border-transparent rounded" 
                                            wire:click="addCourse({{$curso->idcur}})">
                                            <p class="inline-flex font-semibold ">Añadir curso</p>
                                        </button>
                                    </div>
                                @endif
                                <div class="inline">
