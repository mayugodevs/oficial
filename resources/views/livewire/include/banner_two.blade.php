<div class="absolute inset-x-0">
    <div class="rounded-b-xl    bg-bgfondopricingwhite dark:bg-pricingcintadark  ">
        <div class="w-full flex">
            <div class="w-3/4 py-1 dark:bg-green bg-green ">
            </div>
            <div class="w-1/4 py-1 dark:bg-colheaderdark bg-bgprogresb ">
            </div>
        </div>

        <div class="  px-6 py-8  w-full  ">
            <div
                class="font-inter font-bold text-center text-lg dark:text-borderconcours text-txtcommentply">
                Especialidad de Gestión de Operacionesbbbb
            </div>
            <div class="mt-4 ">
                <button
                    class="text-green_especialidad py-2.5 h-12 rounded-lg bg-bgbtngreen w-full  focus:outline-none text-lg font-medium">
                    Empezar especialidad
                </button>
            </div>
            <div class="flex w-full items-center  pt-6">
                <div
                    class=" dark:border-txtcommentply border-t-2 border-borderconcours w-full">
                </div>
                <div class="dark:text-borderconcours text-txtcommentply -mt-1">O</div>
                <div
                    class=" dark:border-txtcommentply border-t-2 border-borderconcours w-full">
                </div>
            </div>
            <div
                class="dark:text-borderconcours text-txtcommentply font-inter font-bold text-sixteen text-center mt-6 px-6 ">
                Sube a Premium y accede a todos los cursos y especialidades
            </div>
            <button
                class="flex bg-morado1 w-full focus:outline-none rounded-lg mt-6 justify-center items-start py-2.5 font-inter font-medium text-lg text-white-100">
                <div>
                    <svg width="21" height="20" viewBox="0 0 21 20" fill="none"
                        xmlns="http://www.w3.org/2000/svg">
                        <path
                            d="M10.4998 6.66709V17.5004M10.4998 6.66709C10.4998 6.66709 10.4998 5.42311 10.4998 5.00043C10.4998 4.07995 11.2459 3.33376 12.1664 3.33376C13.0869 3.33376 13.8331 4.07995 13.8331 5.00043C13.8331 5.9209 13.0869 6.66709 12.1664 6.66709C11.6686 6.66709 10.4998 6.66709 10.4998 6.66709ZM10.4998 6.66709C10.4998 6.66709 10.4998 5.05285 10.4998 4.58376C10.4998 3.43317 9.56701 2.50043 8.41642 2.50043C7.26583 2.50043 6.33309 3.43317 6.33309 4.58376C6.33309 5.73435 7.26583 6.66709 8.41642 6.66709C9.09792 6.66709 10.4998 6.66709 10.4998 6.66709ZM4.66642 10.0004H16.3331M4.66642 10.0004C3.74595 10.0004 2.99976 9.25423 2.99976 8.33376C2.99976 7.41328 3.74595 6.66709 4.66642 6.66709H16.3331C17.2536 6.66709 17.9997 7.41328 17.9997 8.33376C17.9997 9.25423 17.2536 10.0004 16.3331 10.0004M4.66642 10.0004L4.66642 15.8338C4.66642 16.7542 5.41261 17.5004 6.33309 17.5004H14.6664C15.5869 17.5004 16.3331 16.7542 16.3331 15.8338V10.0004"
                            stroke="Currentcolor" stroke-width="1.66667"
                            stroke-linecap="round" stroke-linejoin="round" />
                    </svg>

                </div>
                <div class="pl-2">
                    Subir a Premium
                </div>
            </button>
        </div>
    </div>
</div>