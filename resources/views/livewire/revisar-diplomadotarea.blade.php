<div x-data="modal_alpine2()">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
            @foreach ($tareas as $tarea)
                <tr>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="flex items-center">
                            <input type="number" onkeypress="validar_nota(event, this.value, {{$tarea->dipl_tarea_id}} , {{$tarea->dipl_pagos_id}})" placeholder="poner nota" >                             
                            <button x-on:click="abrirModal" wire:click="show_entrega({{$tarea->dipl_tarea_id}},{{$tarea->dipl_pagos_id}})">Ver</button>
                        </div>
                    </td>                   
                    ...
                    <div class="">
                        @forelse ($entregas as $entrega)
                            <div>
                                ...
                                @endif
                                Mensaje : 
                                <input type="text" value="{{$entrega->feedback}}" onkeypress="validar(event, this.value, {{$entrega->id}})" >
                            </div>
                        @empty
                            vacio
                        @endforelse 
    ...
    @push('js')
        <script>
            ...
            function validar(e, valor, id) {
                tecla = (document.all) ? e.keyCode : e.which;
                if (tecla==13) { 
                    Livewire.emit('save_feedback_esp', valor, id);
                }
            }
            function validar_nota(e, valor, id_tarea, pay_id) {
                tecla = (document.all) ? e.keyCode : e.which;
                if (tecla==13) { 
                    if(valor >=0 && valor <=100) Livewire.emit('save_nota_dip', valor, id_tarea, pay_id ); 
