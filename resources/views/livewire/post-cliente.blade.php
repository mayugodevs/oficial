<div>
        ...
        <div class="flex items-center justify-between w-full my-4 pl-4 sm:pr-4">
            <div class="mr-6">
                <h2 class="text-3xl md:text-4xl font-semibold tracking-tight leading-7 md:leading-10 mb-1 truncate">
                    Manage My Shop</h2>
                <div class="font-base tracking-tight text-gray-600">Lorem ipsum is placeholder text commonly used 
                    in the graphic, print, and publishing industries for previewing layouts and visual mockups.</div>
            </div>
            <div class="flex items-center">
            <button class="bg-gray-900 px-5 py-2 text-sm shadow-sm font-semibold tracking-wider text-white 
            rounded-full hover:bg-gray-800" onclick="openModal('mymodalcentered')">AddShop</button>
            </div>
        </div>
    ...
    @foreach ($posts as $post)
        ...
        @foreach ($post->answers as $answer)
            Respuestas:
            <hr>
            <strong><i>{{$answer->answer}}</i></strong> <hr>
            ...
            {{-- ******************************************** --}}
            {{-- ******************************************** --}}
            Likes : <strong><i>{{$answer->likes}}</i></strong>
            {{-- ******************************************** --}}       
            {{-- ******************************************** --}}       
            {{-- ******************************************** --}}       
            {{-- ******************************************** --}}       
            ...
            <hr>
            Usuario: <x-usuario iduser="{{$answer->user_id}}" />
            <hr>
        @endforeach
    @endforeach
            ...
            <div class="bg-white flex rounded-lg w-1/2 relative">
                <div class="flex flex-col items-start">
                    <div class="p-7 flex items-center w-full">
                        <div class="text-gray-900 font-bold text-lg">Modal Centereddd</div>
                        ...
                        {{-- ********** --}}
                            <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
                                ...
                                <input class="appearance-none block w-full bg-gray-200 text-gray-700 border border-red-500 rounded py-3 px-4
                                    mb-3 leading-tight focus:outline-none focus:bg-white"  type="text" placeholder="Titulo de post" wire:model="postTitle">
                                    @error('postTitle')
                                        <span>{{$message}} </span>
                                    @enderror
                            </div>
                            <div class="w-full md:w-1/2 px-3">
                                ...
                                placeholder="Descripcion de post" wire:model="post" >
                                @error('post')
                                        <span>{{$message}} </span>
                                    @enderror
                            </div>
                    ...
                    <div class="p-7 flex justify-end items-center w-full">
                        <button type="button" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded mr-3" wire:click="createPost">
                            Publicar
                        </button>
                        <button type="button" onclick="modalClose('mymodalcentered')" class="bg-transparent hover:bg-gray-500 text-blue-700 
